...
export function DashboardPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* KPI Cards */}
      {summaryQuery.isError && <p className="text-destructive">Failed to load cost data</p>}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <CardContent>
            {summaryQuery.isLoading ? (
              ...
              <p className="text-2xl font-bold">
                {summaryQuery.data ? formatCurrency(summaryQuery.data.mtd_total) : "—"}
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            {summaryQuery.isLoading ? (
              ...
              <>
                <p className="text-2xl font-bold">
                  {summaryQuery.data ? formatCurrency(summaryQuery.data.projected_month_end) : "—"}
                </p>
                <p className="text-xs text-muted-foreground mt-1">Linear projection</p>
              </>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Prior Month</CardTitle>
          </CardHeader>
          <CardContent>
            {summaryQuery.isLoading ? (
              <div className="animate-pulse bg-muted rounded h-8 w-32" />
              ...
              <>
                <p className="text-2xl font-bold">
                  {summaryQuery.data ? formatCurrency(summaryQuery.data.prior_month_total) : "—"}
                </p>
                {summaryQuery.data && (
                  <div className="mt-1">
        ...
        <Card>
          ...
          <CardContent>
            {anomalySummary.isLoading ? (
              ...
              <>
                <p
                  className={`text-2xl font-bold ${(anomalySummary.data?.active_count ?? 0) > 0 ? "text-destructive" : "text-foreground"}`}
                >
                    ...
                    {(() => {
                      const parts = [
                        (anomalySummary.data.critical_count ?? 0) > 0
                          ? {
                              label: `${anomalySummary.data.critical_count} Critical`,
                              cls: "text-red-600",
                            }
                          : null,
                        (anomalySummary.data.high_count ?? 0) > 0
                          ? {
                              label: `${anomalySummary.data.high_count} High`,
                              cls: "text-orange-500",
                            }
                          : null,
                        (anomalySummary.data.medium_count ?? 0) > 0
                          ? {
                              label: `${anomalySummary.data.medium_count} Medium`,
                              cls: "text-blue-600",
                            }
                          : null,
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base font-semibold">Top 10 Most Expensive Resources</CardTitle>
              ...
              {topResourcesQuery.data && topResourcesQuery.data.length > 0 ? (
                topResourcesQuery.data.map((resource) => (
                  <TableRow key={resource.resource_id || resource.resource_name}>
                    ...
                    <TableCell className="text-right font-mono">
                      $
                      {resource.total_cost.toLocaleString("en-US", {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}
                    </TableCell>

...
describe('useSpendSummary', () => {
  it('returns spend summary data after loading', async () => {
    ...
    expect(result.current.data?.projected_month_end).toBe(
      mockSpendSummary.projected_month_end
    );
    expect(result.current.data?.prior_month_total).toBe(
      mockSpendSummary.prior_month_total
    );
    expect(result.current.data?.mom_delta_pct).toBe(
      mockSpendSummary.mom_delta_pct
    );
  });
});
...
describe('useSpendTrend', () => {
  it('returns daily spend trend data after loading', async () => {
    ...
    expect(result.current.data?.[0].usage_date).toBe(
      mockTrendData[0].usage_date
    );
    expect(result.current.data?.[0].total_cost).toBe(
      mockTrendData[0].total_cost
    );
  });
});
...
describe('useSpendBreakdown', () => {
  it('returns breakdown data after loading', async () => {
    const { result } = renderHook(
      () => useSpendBreakdown('service_name', 30),
      { wrapper: makeWrapper() }
    );
    ...
    expect(result.current.data?.[0].dimension_value).toBe(
      mockBreakdownData[0].dimension_value
    );
    expect(result.current.data?.[0].total_cost).toBe(
      mockBreakdownData[0].total_cost
    );
  });
});
...
describe('useTopResources', () => {
  it('returns top resources data after loading', async () => {
    ...
    expect(result.current.data?.[0].resource_name).toBe(
      mockTopResources[0].resource_name
    );
    expect(result.current.data?.[0].total_cost).toBe(
      mockTopResources[0].total_cost
    );
  });
});
